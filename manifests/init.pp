class rjil {
  @rjil::localuser { 'soren':
    realname => 'Soren Hansen',
    sshkeys  => "ssh-rsa  soren@bulky\n",
  }

  @rjil::localuser { 'rohit':
    realname => 'Rohit Karajgi',
    sshkeys  => "ssh-rsa  rohit@rocloud\n",
  }

  @rjil::localuser { 'amar':
    realname => 'Amar Krishna',
    sshkeys  => "ssh-rsa  amar@localhost.localdomain\n",
  }

  @rjil::localuser { 'sanjayu':
    realname => 'Sanjay S Upadhyay',
    sshkeys  => "ssh-dss  sanjayu@glowfriend.com\nssh-rsa  sanjayu@glowfriend.com\n",
  }

  @rjil::localuser { 'omp':
    realname => 'Om Prakash Pandey',
    sshkeys  => "ssh-rsa \n",
  }

  @rjil::localuser { 'jas':
    realname => 'Jaspreet Walia',
    sshkeys  => "ssh-rsa \n",
  }

  @rjil::localuser { 'rushiagr':
    realname => 'Rushi Agrawal',
    sshkeys  => "ssh-rsa  rushi.agr@gmail.com\n",
  }


  @rjil::localuser { 'vletchumanan':
    realname => 'Valliappan Letchumana',
    sshkeys  => "ssh-rsa \n",
  }


  @rjil::localuser { 'ashish-chandra':
    realname => 'Ashish Chandra',
    sshkeys  => "ssh-rsa \n",
  }

  @rjil::localuser { 'saju':
    realname => 'Saju Madhavan',
    sshkeys  => "ssh-rsa  saju@saju-Inspiron-5521\n",
  }

  @rjil::localuser { 'angrybird':
    realname => 'Maninder Rajpal',
    sshkeys  => "ssh-rsa \n",
  }
}
